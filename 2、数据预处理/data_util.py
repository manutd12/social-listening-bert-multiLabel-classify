...
class data(object):
    def get_label(self,input_file, cut_labels):
        '''
          获取数据集的所有的label
        :param input_file: execl文件，包含数据和对应的label
        :param cut_labels: 去除某些类别的数据
        :return:
            列表：所有的label
            字典：key:小写后的标签 value:原始标签

        '''
        labels = self._read_excel(input_file,2,sheet_name="Taxonomy")
        labelmap = dict()
        lower2originalLabel = dict()
    ...
    def process_text(self,content,stopwords):
        ...
        return content_list
    def get_data(self,input_file, all_labels, stopwords_file):
        '''

        :param input_file: excel文件，包含数据和对应的label
        :param all_labels: 数据集的label集合
        :param stopwords_file: 停用词文件
        :return:
            docs：经过处理后的句子集合
            labs：句子对应的label集合
            original_docs：处理之前的原句子集合
        '''
        ...
        for i,content in enumerate(text):
            ...
            if len(label_list) ==0:
                continue
            docs.append(content_list)  # 经过处理后的句子
            labs.append(label_list)  # 句子的label
            original_docs.append(content)  # 处理之前的原句子
